...
export default function ChatPage() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { id: uuidv4(), text: 'Welcome to AI Career Coach — ask career, resume, or interview questions.', role: 'system', time: new Date().toLocaleTimeString() }
    ])
    ...
    const onSend = async () => {
        ...
        const userMsg: Message = { id: uuidv4(), text, role: 'user', time: new Date().toLocaleTimeString() }
        ...
        try {
            ...
            if (!res.ok) {
                ...
                setMessages(prev => [...prev, { id: uuidv4(), text: `Error: ${errMsg}`, role: 'assistant', time: new Date().toLocaleTimeString() }])
                return
            }
            ...
            setMessages(prev => [...prev, { id: uuidv4(), text: assistantText, role: 'assistant', time: new Date().toLocaleTimeString() }])
        } catch (err: any) {
            setMessages(prev => [...prev, { id: uuidv4(), text: `Network error: ${err?.message ?? String(err)}`, role: 'assistant', time: new Date().toLocaleTimeString() }])
        } finally {
            setSending(false)
        }
    }
